...
from common.tools import create_image_mat


def synthesize_graphon(r: int = 1000, type_idx: Union[str, int] = 0) -> np.ndarray:
    """
    Synthesize graphons
    ...
    if isinstance(type_idx, str):
        graphon = getattr(graphons, type_idx)
        w = create_image_mat(graphon, 1000, False).astype(np.float64)
    elif type_idx == 0:
        u = u[::-1, :]
        v = v[:, ::-1]
        w = u @ v
...
def simulate_graphs(w: np.ndarray, num_graphs: int = 10,
    ...
    if graph_size == 'fixed':
        ...
        raise RuntimeError(f'{graph_size}')
    # print(numbers)
