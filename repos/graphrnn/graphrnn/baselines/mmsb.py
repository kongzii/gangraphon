...
def disjoint_cliques_test_graph(num_cliques, clique_size):
    G = nx.disjoint_union_all(
        [nx.complete_graph(clique_size) for _ in range(num_cliques)]
    )
...
def mmsb(N, K, data):
    ...
    inference.initialize(
        optimizer=tf.train.AdamOptimizer(learning_rate=0.01), n_iter=n_iter
    )
...
if __name__ == "__main__":
    ...
    utils.save_graph_list(
        gen_graphs, os.path.join(save_path, prog_args.dataset + ".dat")
    )
