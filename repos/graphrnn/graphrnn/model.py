...
def sample_sigmoid(y, sample, thresh=0.5, sample_time=2):
    ...
    if sample:
        if sample_time > 1:
            y_result = Variable(torch.rand(y.size(0), y.size(1), y.size(2))).to(
                y.device
            )
            ...
            y_thresh = Variable(torch.rand(y.size(0), y.size(1), y.size(2))).to(
                y.device
            )
            y_result = torch.gt(y, y_thresh).float()
    # do max likelihood based on some threshold
...
def sample_sigmoid_supervised(y_pred, y, current, y_len, sample_time=2):
    ...
    # do sampling
    y_result = Variable(torch.rand(y_pred.size(0), y_pred.size(1), y_pred.size(2))).to(
        y.device
    )
    # loop over all batches
    for i in range(y_result.size(0)):
        ...
        if current < y_len[i]:
            while True:
                y_thresh = Variable(torch.rand(y_pred.size(1), y_pred.size(2))).to(
                    y.device
                )
...
class GRU_plain(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return Variable(torch.zeros(self.num_layers, batch_size, self.hidden_size)).to(
            next(self.rnn.parameters()).device
        )
...
class MLP_VAE_plain(nn.Module):
    ...
    def forward(self, h):
        ...
        eps = Variable(torch.randn(z_sgm.size())).to(z_mu.device)
        z = eps * z_sgm + z_mu
        # decoder
        y = self.decode_1(z)
        y = self.relu(y)
...
class MLP_VAE_conditional_plain(nn.Module):
    ...
    def forward(self, h):
        ...
        # reparameterize
        z_sgm = z_lsgms.mul(0.5).exp_()
        eps = Variable(torch.randn(z_sgm.size(0), z_sgm.size(1), z_sgm.size(2))).to(
            z_mu.device
        )
        z = eps * z_sgm + z_mu
        # decoder
