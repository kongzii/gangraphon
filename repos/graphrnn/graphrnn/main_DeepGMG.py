...
def adjacency_list(adj_generator):
    return list(map(lambda t: list(t[1].keys()), adj_generator))
...
def train_DGMG_epoch(epoch, args, model, dataset, optimizer, scheduler, is_fast=False):
    ...
    for i in order:
        ...
        node_count = 1
        node_embedding = [
            Variable(torch.ones(1, args.node_embedding_size)).to(device)
        ]  # list of torch tensors, each size: 1*hidden

        loss = 0
        while node_count <= graph.number_of_nodes():
            print("Graph:", list(graph.adjacency()))
            node_neighbor = adjacency_list(
                graph.subgraph(list(range(node_count))).adjacency()
            )  # list of lists (first node is zero)
            print(node_neighbor)

            node_neighbor_new = adjacency_list(
                graph.subgraph(list(range(node_count + 1))).adjacency()
            )[
                -1
            ...
            if node_count < graph.number_of_nodes():
                ...
                loss_addnode_step = F.binary_cross_entropy(
                    p_addnode, Variable(torch.ones((1, 1))).to(device)
                )
                ...
                # calc loss
                loss_addnode_step = F.binary_cross_entropy(
                    p_addnode, Variable(torch.zeros((1, 1))).to(device)
                )
                # loss_addnode_step.backward(retain_graph=True)
                loss += loss_addnode_step
                loss_addnode += loss_addnode_step.data
            ...
            while edge_count <= len(node_neighbor_new):
                ...
                if edge_count < len(node_neighbor_new):
                    ...
                    loss_addedge_step = F.binary_cross_entropy(
                        p_addedge, Variable(torch.ones((1, 1))).to(device)
                    )
                    ...
                    # calc loss
                    loss_addedge_step = F.binary_cross_entropy(
                        p_addedge, Variable(torch.zeros((1, 1))).to(device)
                    )
                    # loss_addedge_step.backward(retain_graph=True)
                    loss += loss_addedge_step
                    loss_addedge += loss_addedge_step.data
...
if __name__ == "__main__":
    ...
    # remove self loops
    for graph in graphs:
        edges_with_selfloops = list(nx.selfloop_edges(graph))
