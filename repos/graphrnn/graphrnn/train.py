...
import graphrnn.create_graphs as create_graphs
...
def train_vae_epoch(
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        log_value(
            ...
            loss_bce.item().detach().numpy(),
            epoch * args.batch_ratio + batch_idx,
        )
        log_value(
            "kl_loss_" + args.fname,
            loss_kl.item().detach().numpy(),
            epoch * args.batch_ratio + batch_idx,
        )
        log_value(
...
def test_rnn_epoch(epoch, args, rnn, output, test_batch_size=16):
    ...
    for i in range(max_num_node):
        ...
        x_step = Variable(torch.zeros(test_batch_size, 1, args.max_prev_node)).to(
            device
        )
    ...
    while epoch <= args.epochs:
        ...
        if epoch % args.epochs_test == 0 and epoch >= args.epochs_test_start:
            for sample_time in range(1, 4):
                ...
                # save graphs
                # fname = args.graph_save_path + args.fname_pred + str(epoch) +'_'+str(sample_time) + '.pickle'
                Path(args.output_predictions).mkdir(exist_ok=True, parents=True)
                fname = args.output_predictions + f"/epoch={epoch}.pickle"
