...
def _gaussian_kernel(x, y, ux, uy, sigma):
    ...
    return (
        1
        / (2 * np.pi * sigma ** 2)
        * np.exp(-((x - ux) ** 2 + (y - uy) ** 2) / (2 * sigma ** 2))
    )
...
class PersistenceImage:
    ...
    def transform(self, persistence_diagrams, max_persistence=None):
        ...
        return self._transform([pd for pd in persistence_diagrams], max_persistence)
    ...
    def _transform(self, persistence_diagram, max_persistence):
        ...
        xc, yc = np.meshgrid(
            np.cumsum(xstep) - 0.5 * xstep, np.cumsum(ystep) - 0.5 * ystep
        )
        ...
        for i in range(self._width):
            for j in range(self._height):
                ...
                for c, d in persistence_diagram:
                    f = PersistenceImage.w(c, d, max_persistence) * _gaussian_kernel(
                        cx, cy, c, d, 0.2
                    )
